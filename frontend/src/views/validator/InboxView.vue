  ...
  <div class="p-6 space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        ...
        <p class="font-cuerpo text-sm text-gray-500 mt-1">
          Formularios y cargas Excel para revisión y validación
        </p>
      </div>
    </div>
    ...
    <div class="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
      <button
        ...
        class="px-4 py-2 rounded-lg text-sm font-cuerpo font-medium transition"
        :class="activeTab === 'forms'
          ? 'bg-white text-ubpd-gris shadow-sm'
          : 'text-gray-500 hover:text-ubpd-gris'"
      >
        Formularios
        <span v-if="total > 0" class="ml-1.5 text-xs bg-ubpd-teal/10 text-ubpd-teal px-1.5 py-0.5 rounded-full">
          {{ total }}
        </span>
      </button>
      <button
        @click="activeTab = 'lotes'; loadLotes()"
        class="px-4 py-2 rounded-lg text-sm font-cuerpo font-medium transition"
        :class="activeTab === 'lotes'
          ? 'bg-white text-ubpd-gris shadow-sm'
          : 'text-gray-500 hover:text-ubpd-gris'"
      >
        Carga Excel
        <span v-if="lotes.length > 0" class="ml-1.5 text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full">
          {{ lotes.length }}
        </span>
      </button>
    </div>

    <!-- ── TAB: Formularios individuales ── -->
    <template v-if="activeTab === 'forms'">
      ...
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div v-if="totalPages > 1" class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
            ...
            <div class="flex gap-1">
              <button @click="goToPage(page - 1)" :disabled="page === 1"
                class="p-2 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-40 transition">
        ...
        <div v-else class="py-16 text-center">
          ...
          <p class="font-cuerpo text-sm text-gray-400 mt-1">No hay formularios pendientes de revisión</p>
        </div>
      </div>
    </template>

    <!-- ── TAB: Carga Excel / Lotes ── -->
    <template v-else>
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div v-if="loadingLotes" class="divide-y divide-gray-50">
          <div v-for="i in 3" :key="i" class="px-6 py-5 flex items-center gap-4 animate-pulse">
            <div class="flex-1 space-y-2">
              <div class="w-48 h-4 rounded bg-gray-200" />
              <div class="w-32 h-3 rounded bg-gray-200" />
            </div>
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="font-cuerpo text-xs text-gray-400 mt-0.5">
                {{ lote.dependencia_nombre }} · {{ lote.usuario_nombre }}
              </p>
            </div>

            <span class="flex-shrink-0 inline-flex items-center gap-1 font-cuerpo text-xs font-semibold
                         px-2.5 py-1 rounded-full bg-amber-50 text-amber-700">
              {{ lote.total_registros }} registros
            </span>

            <div class="flex-shrink-0 text-right hidden sm:block">
              <p class="font-cuerpo text-sm text-gray-600">{{ formatDate(lote.fecha_carga) }}</p>
              <p class="font-cuerpo text-xs text-gray-400">{{ formatTime(lote.fecha_carga) }}</p>
            </div>
        ...
        <div v-else class="py-16 text-center">
          ...
          <p class="font-subtitulo font-semibold text-ubpd-gris">Sin cargas pendientes</p>
          <p class="font-cuerpo text-sm text-gray-400 mt-1">No hay lotes Excel pendientes de validación</p>
        </div>
      </div>
    </template>
...
const activeTab = ref<'forms' | 'lotes'>('forms')
...
const totalPending = computed(() => total.value + lotes.value.length)
const totalPages = computed(() => Math.max(1, Math.ceil(total.value / pageSize)))

// Lotes Excel
const loadingLotes = ref(false)
const lotes = ref<LoteResumen[]>([])
...
async function loadForms() {
  ...
  try {
    ...
    // Excluir los que tienen lote_excel_id (van al tab de lotes)
    const data = await get<PendingResponse>(`/validation/pending?${params.toString()}`)
    // Filtrar formularios que NO son de carga Excel (para no duplicar con el tab de lotes)
    forms.value = data.items.filter((f: any) => !f.lote_excel_id)
...
async function loadLotes() {
  ...
  try {
    lotes.value = await get<LoteResumen[]>('/validation/lotes')
  } catch {
    lotes.value = []
  } finally {
...
function goToBatch(loteId: string) { router.push(`/validator/review-batch/${loteId}`) }
